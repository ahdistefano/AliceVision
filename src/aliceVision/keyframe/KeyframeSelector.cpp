...
double KeyframeSelector::computeSharpness(const cv::Mat& grayscaleImage, const std::size_t windowSize)
{
    ...
    for (int y = 0; y < sum.rows - windowSize; ++y) {
        for (int x = 0; x < sum.cols - windowSize; ++x) {
                                      ...
                                      const cv::Mat& previousGrayscaleImage, const std::size_t cellSize)
{
    ...
    for (std::size_t y = 0; y < sumflow.size().height; y += cellSize) {
        ...
        for (std::size_t x = 0; x < sumflow.size().width; x += cellSize) {
            cv::Point2d tl = sumflow.at<cv::Point2d>(y, x);
            cv::Point2d tr = sumflow.at<cv::Point2d>(y, x + cellSize - 1);
            cv::Point2d bl = sumflow.at<cv::Point2d>(y + maxCellSizeHeight - 1, x);
            cv::Point2d br = sumflow.at<cv::Point2d>(y + maxCellSizeHeight - 1, x + cellSize - 1);
